...
class FederationFetchHandler(BaseEndpoint):
    def __init__(
        self,
        config: dict,
    ...
    def _build_payload(self) -> dict:
        ...
        return entity.entity_configuration_as_dict

    def endpoint(self, context: Context) -> Redirect | Response:
        requested_sub = context.qs_params.get("sub", "")
        if not requested_sub:
            data = json.dumps({"error": "invalid_request", "error_description": "missing sub parameter"})
            return Response(data, status="400", content="application/json")

        if _normalize_entity_id(requested_sub) != _normalize_entity_id(self._subject):
