...
async def test_eidas_get_page_redirects_to_idps(auth_client, db_session):
    """La pagina dedicata /admin/eidas non esiste più: eIDAS è una sezione di /admin/idps."""
    response = await auth_client.get("/admin/eidas", follow_redirects=False)
    assert response.status_code == 301
    assert response.headers["location"] == "/admin/idps#eidas"
...
async def test_eidas_enable_flow(auth_client, db_session):
    ...
    assert response.headers["location"] == "/admin/idps?saved=1#eidas"
